...
async def disconnect(sid):
    ...
    _currentConnections -= 1
    print('App disconnected with ID', sid)
    print('Current connections ->', _currentConnections)
...
async def alertAppOfDetection(detection):
    ...
    await sio.emit('detection', {'data': detection, 'image': _img_base64})
...
async def detection(request):
    ...
    detectionRes = await request.json()
    # print(request)
    print(await request.json())
    # print(request.json)

    # Server will only check first detection and then each 50th frame thereafter
    if abs(detectionRes['frame_id'] - lastDetectedFrame) > 50 or lastDetectedFrame == 0:
        ...
        if abs(detectionRes['frame_id'] - lastDetectedFrame) > 100:
            previousDetections = []

        lastDetectedFrame = detectionRes['frame_id']
        ...
        animalCounters = {}
        for detectedAnimal in detectionRes['objects']:
        ...
        for detectedAnimal in animalCounters:
            ...
            if animalCounters[detectedAnimal]['count'] > 1:
                ...
                if not animalAlreadyDetected:
                    ...
                    print('\tFrame ->', detectionRes['frame_id'])
                    print('\tHerd of ->', detectedAnimal, '\n')

                    await alertAppOfDetection(detectionRes)
...
app.router.add_post('/detection', detection)
